"""
パートナー共有画面 - 共有コード生成・双方向メッセージ
"""
import os

import streamlit as st

from utils.calculations import to_jst_str
from utils.supabase_client import (
    get_partner_share,
    create_partner_share,
    deactivate_partner_share,
    add_partner_message,
    get_partner_messages,
)

st.set_page_config(page_title="パートナー共有", page_icon="👫", layout="centered")

st.title("👫 パートナー共有")
st.caption("禁煙の進捗をパートナーと共有しましょう")

# ─── 共有URLのベース取得 ────────────────────────────────────────────────────
app_url = os.environ.get("APP_URL", "").rstrip("/")

# ─── 現在の共有状態を取得 ───────────────────────────────────────────────────
share = get_partner_share()

# ─── 共有コード生成・停止UI ─────────────────────────────────────────────────
st.markdown("---")
st.subheader("🔗 共有設定")

if share:
    share_code = share["share_code"]

    if app_url:
        share_url = f"{app_url}/?share={share_code}"
        st.success(f"✅ 共有コード: **{share_code}**")
        st.markdown("**パートナーに以下のURLを共有してください：**")
        st.code(share_url, language=None)
        st.caption("このURLを開くとパートナーが禁煙の進捗を閲覧できます。")
    else:
        st.success(f"✅ 共有コード: **{share_code}**")
        st.info(
            "パートナーにアクセスしてもらうには、アプリのURLの末尾に "
            f"`?share={share_code}` を付けてください。\n\n"
            "例: `https://your-app.streamlit.app/?share={share_code}`\n\n"
            "環境変数 `APP_URL` にデプロイ先URLを設定するとURLが自動生成されます。"
        )

    if st.button("🛑 共有を停止する", use_container_width=True, type="secondary"):
        deactivate_partner_share()
        st.success("共有を停止しました。")
        st.rerun()
else:
    st.info("共有コードを生成して、パートナーに禁煙の進捗を見せましょう。")
    if st.button("🔑 共有コードを生成する", use_container_width=True, type="primary"):
        share = create_partner_share()
        st.rerun()

# ─── メッセージ送受信 ───────────────────────────────────────────────────────
if share:
    share_code = share["share_code"]

    st.markdown("---")
    st.subheader("💬 パートナーへのメッセージ")

    # メッセージ送信フォーム
    with st.form("send_message_form", clear_on_submit=True):
        message_text = st.text_area(
            "パートナーへのメッセージ",
            placeholder="今日も頑張っているよ！",
            max_chars=500,
        )
        send_btn = st.form_submit_button("送信する 📨", use_container_width=True)

    if send_btn and message_text.strip():
        add_partner_message(share_code, "user", message_text.strip())
        st.success("✅ メッセージを送信しました！")
        st.rerun()
    elif send_btn:
        st.warning("メッセージを入力してください。")

    # メッセージ一覧
    messages = get_partner_messages(share_code)
    if messages:
        st.markdown("---")
        st.subheader("📩 メッセージ履歴")
        for msg in messages:
            sent_at = to_jst_str(msg["sent_at"])  # JST に変換して表示
            if msg["sender"] == "user":
                with st.chat_message("user"):
                    st.markdown(msg["message"])
                    st.caption(f"あなた · {sent_at}")
            else:
                with st.chat_message("assistant"):
                    st.markdown(msg["message"])
                    st.caption(f"パートナー · {sent_at}")
    else:
        st.info("まだメッセージはありません。最初のメッセージを送りましょう！")

# ─── 使い方説明 ──────────────────────────────────────────────────────────────
st.markdown("---")
with st.expander("📖 使い方"):
    st.markdown("""
**パートナー共有の使い方：**

1. **共有コードを生成** → パートナーに共有URLを送る
2. パートナーが共有URLを開くと **閲覧専用ダッシュボード** が表示される
   - 禁煙期間・節約金額・マイルストーン・妊活チェック状況を確認できる
3. パートナーはダッシュボードから **応援メッセージ** を送ることができる
4. このページでパートナーからのメッセージを確認できる

**プライバシーについて：**
- 共有を停止すると新しいコードが必要になります
- パートナーは閲覧と応援メッセージ送信のみ可能です（データの変更はできません）
""")
