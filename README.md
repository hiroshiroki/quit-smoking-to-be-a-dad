...
| 画面 | 機能 |
|------|------|
| 🏠 ダッシュボード | 禁煙日数・節約金額のリアルタイム表示、マイルストーン進捗、本日のチェック状況、今日から始めるクイックスタート |
| 🚭 禁煙トラッカー | 「吸いたい」衝動ログ、緊急回避モード（5分タイマー・深呼吸ガイド）、再禁煙サポート、挑戦回数トラッキング、トリガー別対処法のリアルタイム表示 |
| 🌿 妊活チェック | 亜鉛・葉酸・睡眠・運動・ストレスの日次チェックリスト |
| 💌 日記 | 未来の子どもへのメッセージ投稿・閲覧 |
| ⚙️ 設定 | 禁煙開始日・1日の本数・タバコ価格の設定、Discord通知設定、トリガー別対処法の登録 |
...
> **既存環境へのアップデート：** すでに `smoke.user_settings` テーブルが存在する場合は、以下の ALTER TABLE も実行してください。
>
> ```sql
> ALTER TABLE smoke.user_settings
>   ADD COLUMN IF NOT EXISTS quit_datetime TIMESTAMPTZ;
> ```
>
> この列が追加されると、「今日から禁煙スタート！」ボタンを押した正確な時刻から禁煙期間が計算されます。
...
アプリ起動後、ダッシュボードに表示される **「今日から禁煙スタート！」** ボタンを使うと、1日の本数と1箱の価格を入力するだけですぐに始められます。

詳細な設定（禁煙開始日の変更・箱あたり本数など）は **設定** 画面（サイドバーの「4_設定」）から行えます。
...
1. **🆘 緊急回避モード**（ページ上部のエキスパンダー）
   - **5分タイマー** — 「衝動のピークは5分で過ぎる」原則を利用したカウントダウンタイマーです。
   - **深呼吸ガイド（ボックス呼吸）** — 4秒吸う→4秒止める→4秒吐く→4秒止める のアニメーションガイドです。
   - **行動リスト** — 水を飲む・歯磨き・散歩など、すぐできる代替行動の一覧です。

2. **衝動の記録**
   - **きっかけ（トリガー）** を選ぶと、設定画面で登録した **対処法がその場に表示** されます。
   - 衝動の強さ（1〜5）・結果（我慢 or 喫煙）を選択して「記録する」をクリックします。
   - きっかけの「その他」を選ぶと自由にテキスト入力できます。
   - 気持ちを落ち着かせるために未来の子どもへのひとことも書けます。

3. **再禁煙サポート**
   - 「吸ってしまった」と記録した直後に **再スタートUI** が表示されます。
   - これまでの禁煙日数を確認でき、「今日から再スタートする」ボタンで禁煙開始日をリセットできます。
...
7. **挑戦履歴**
   - 禁煙に挑戦した回数・各回の継続日数・過去最長記録を確認できます。
   - 現在の挑戦が過去最長を更新中の場合はバッジで通知します。
...
- 食後・ストレス・仕事の合間など、8つのトリガーそれぞれに自分の対処法を登録できます。
- 禁煙トラッカーでトリガーを選ぶと、登録した対処法がその場にリアルタイム表示されます。
...
### 禁煙失敗サポート機能（完了）
- **緊急回避モード** — 5分タイマー・深呼吸ガイド（ボックス呼吸）・行動リストをワンタップで表示
- **再禁煙サポート（リラプス対応）** — 「吸ってしまった」記録後に今日からの再スタートをサポート
- **挑戦回数トラッキング** — 何回目の挑戦か・過去最長記録・最長更新中バッジを表示
- **トリガー別コーピング戦略** — 設定画面で登録した対処法をトリガー選択時にリアルタイム表示
- **クイックスタート** — 設定未入力状態のホーム画面から本数と価格を入力するだけで即スタート可能

### バグ修正・精度改善（完了）
- **再スタートボタンのバグ修正** — 「吸ってしまった」記録後の「今日から再スタートする」ボタンが機能しなかった問題を修正（Streamlit の `session_state` を使ってフォーム外で管理）
- **禁煙期間の正確な時刻計算** — 禁煙開始日時（`quit_datetime`）を秒単位で記録し、「今日の0時」ではなく**ボタンを押した正確な時刻**から経過時間を計算
- **1時間未満の分単位表示** — 開始直後は「○時間」ではなく「○分」で表示
- **Python 3.10 のタイムスタンプ互換対応** — Supabase が返すマイクロ秒が5桁のタイムスタンプを正規化してパース（`_parse_ts` ヘルパー追加）
